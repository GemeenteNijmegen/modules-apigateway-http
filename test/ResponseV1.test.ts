import { Response } from '../src/V1/Response';
...
describe('ok', () => {
  ...
  test('300', () => {
    expect(() => Response.ok(300)).toThrow('Only 2xx statuscodes are allowed');
  });

  test('199', () => {
    expect(() => Response.ok(199)).toThrow('Only 2xx statuscodes are allowed');
  });
...
describe('html', () => {
  ...
  test('200 without cookies', () => {
    ...
    expect(r1.statusCode).toBe(200);
    expect(r1.headers!.Cookie).toBeUndefined();
  ...
  test('custom status with single cookie', () => {
    ...
    expect(r2.headers!.Cookie).toContain(cookie1);
  });

  test('with multiple cookie', () => {
    const r3 = Response.html('<html></html>', 200, [cookie1, cookie2]);
    expect(r3.headers!.Cookie).toContain(cookie1);
    expect(r3.headers!.Cookie).toContain(cookie2);
    expect(r3.headers!.Cookie).toContain(`${cookie1}; ${cookie2}`);
  });
